...
# --- SECRET_KEY: 환경변수 기반, 없으면 임시 키 생성(개발용) ---
...
if not db_url:
    # SQLite 폴백: instance 폴더 사용 (Flask 표준 경로)
...
def init_db():
    """기본 카운트 레코드 생성"""
    db.create_all()
    types = ["ghost", "dopamine", "soloplayer",
             "humanlatte", "muscler", "hotnevi", "total"]
    for t in types:
...
# 앱 import 시 한 번만 초기화 (배포 환경에서는 별도 init 스크립트 권장)
try:
...
questions = [
    {  # 1
        ...
        "options": [
            {
                "text": "오늘만은 도파민의 노예!😵‍💫\n밀렸던 드라마, 영화, 유튜브를 본다.",
                "tags": ["in", "static", "online"],
            },
            {
                "text": "오늘이 날이다.\n만나지 못했던 친구와 당장 만난다.",
                "tags": ["out", "dynamic", "offline"],
            },
        ],
    },
    {  # 2
...
# --- 유형 방향 벡터 (연속형 알고리즘용) ---
TYPE_VECTORS = {
    "ghost": (-1.0, -1.0, -1.0),       # in, static, offline
    "dopamine": (-1.0, -1.0, 1.0),     # in, static, online
    "soloplayer": (-1.0, 1.0, -1.0),   # in, dynamic, offline
    "humanlatte": (1.0, -1.0, -1.0),   # out, static, offline
    "muscler": (1.0, 1.0, -1.0),       # out, dynamic, offline
    "hotnevi": (1.0, 1.0, 1.0),        # out, dynamic, online
}


def compute_axis_ranges():
    """
    각 축(dp: out-in, da: dynamic-static, dd: online-offline)에 대해
    가능한 최소/최대 합을 기준으로 정규화 범위를 계산.
    """
    dp_min = dp_max = 0.0
    da_min = da_max = 0.0
    dd_min = dd_max = 0.0
    ...
    for idx, q in enumerate(questions):
        ...
        for opt in q["options"]:
            tags = opt.get("tags", [])
            in_cnt = 1 if "in" in tags else 0
            out_cnt = 1 if "out" in tags else 0
            static_cnt = 1 if "static" in tags else 0
            dynamic_cnt = 1 if "dynamic" in tags else 0
            online_cnt = 1 if "online" in tags else 0
            offline_cnt = 1 if "offline" in tags else 0

            dp = w * (out_cnt - in_cnt)
            da = w * (dynamic_cnt - static_cnt)
            dd = w * (online_cnt - offline_cnt)

            option_dps.append(dp)
            option_das.append(da)
            option_dds.append(dd)

        if option_dps:
            dp_min += min(option_dps)
            dp_max += max(option_dps)
        if option_das:
            da_min += min(option_das)
            da_max += max(option_das)
        if option_dds:
            dd_min += min(option_dds)
            dd_max += max(option_dds)

    place_range = max(abs(dp_min), abs(dp_max)) or 1.0
    activity_range = max(abs(da_min), abs(da_max)) or 1.0
    digital_range = max(abs(dd_min), abs(dd_max)) or 1.0

    return place_range, activity_range, digital_range


PLACE_RANGE, ACTIVITY_RANGE, DIGITAL_RANGE = compute_axis_ranges()


def classify_type(scores: dict) -> str:
    """
    가중치가 적용된 점수(scores)를 받아
    연속형 3축 벡터(dp, da, dd) 기반으로
    6가지 유형 중 가장 가까운 유형을 반환.
    """
    dp = scores["out"] - scores["in"]
    da = scores["dynamic"] - scores["static"]
    dd = scores["online"] - scores["offline"]

    dp_norm = dp / PLACE_RANGE if PLACE_RANGE else 0.0
    da_norm = da / ACTIVITY_RANGE if ACTIVITY_RANGE else 0.0
    dd_norm = dd / DIGITAL_RANGE if DIGITAL_RANGE else 0.0

    best_type = None
    best_dist_sq = None

    for t_name, (vx, vy, vz) in TYPE_VECTORS.items():
        dist_sq = (
            (dp_norm - vx) ** 2
            + (da_norm - vy) ** 2
            + (dd_norm - vz) ** 2
...
def answer(q_id, option_index):
    """
    현재는 GET으로 동작 (기존 템플릿 호환 유지).
    향후에는 POST + CSRF 토큰으로 전환하는 것이 바람직함.
    """
    try:
...
def calculate_result():
    ...
    for q_id_str, option_index in user_answers.items():
        ...
        try:
            ...
            opt = question["options"][option_index]
            tags = opt.get("tags", [])
        except (IndexError, KeyError):
            continue

        weight = QUESTION_WEIGHTS[q_id - 1]
    ...
    # --- 연속형 알고리즘으로 최종 유형 결정 ---
    final_type_name = classify_type(scores)
    if final_type_name not in result_data:
        final_type_name = "ghost"  # 안전장치

    # --- DB 저장 로직 (경합 조건 방지: UPDATE ... SET count = count + 1) ---
    try:
        # 유형 카운트 +1
        db.session.execute(
            update(Counts)
            .where(Counts.type == final_type_name)
            .values(count=Counts.count + 1)
        )
        # 전체 카운트 +1
        db.session.execute(
            update(Counts)
            .where(Counts.type == "total")
            .values(count=Counts.count + 1)
        )
        db.session.commit()
    except Exception as e:
        app.logger.error(f"DB Error in calculate_result: {e}")
        db.session.rollback()
...
def result(type_name):
    ...
    try:
        ...
        app.logger.error(f"DB Read Error in result(): {e}")
...
def admin_stats():
    ...
    input_pw = request.args.get("pw") or ""

    # ADMIN_PASSWORD가 설정되지 않았거나, 비밀번호가 틀리면 403
    if not ADMIN_PW or not secrets.compare_digest(input_pw, ADMIN_PW):
        return "<h1>Access Denied</h1><p>접근 권한이 없습니다.</p>", 403

    try:
        type_stats = db.session.scalars(
            db.select(Counts)
            .filter(Counts.type != "total")
            .order_by(Counts.type)
        ).all()
        total_stat = db.session.get(Counts, "total")
    except Exception as e:
        app.logger.error(f"Admin Stats DB Error: {e}")
